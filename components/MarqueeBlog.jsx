...
export default function MarqueeBlog({postData}){
    ...
    return (
        <>
            ...
            <div className="overflow-hidden w-full mt-7 relative max-w-7xl mx-auto" onMouseEnter={() => setStopScroll(true)} onMouseLeave={() => setStopScroll(false)}>
                ...
                <div className="marquee-inner flex w-fit" style={{ animationPlayState: stopScroll ? "paused" : "running", animationDuration: cardData.length * 2500 + "ms" }}>
                    <div className="flex">
                        {[...postData,...postData].map((card, index) => (
                            <Link href={`/blog/${card.id}`} key={index}>
                            <div  className="w-56 mx-4 h-[20rem] relative group hover:scale-90 transition-all duration-300">
                                <img src={card.thumbnailimage} alt="card" className="w-full h-full object-cover" />
                                <div className="flex items-center justify-center px-4 opacity-0 group-hover:opacity-100 transition-all duration-300 absolute bottom-0 backdrop-blur-md left-0 w-full h-full bg-black/20">
                                   <div>
                                     <p className="text-white text-lg font-semibold text-center">{card.title}</p>
                                       <p className="text-white text-lg font-semibold text-center line-clamp-2">{card.subtitle}</p>
                                   </div>
