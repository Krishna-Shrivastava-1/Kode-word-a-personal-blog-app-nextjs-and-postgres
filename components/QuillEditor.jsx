...
export default function QuillEditor({ content = '', onChange, isEditMode = false }) {
  ...
  // ─── Video handler — file upload only, no confirm dialog ─────────────────
  ...
  // ─── YouTube / Vimeo embed handler ────────────────────────────────────────
  const youtubeHandler = useCallback(() => {
    const quill = quillRef.current?.getEditor()
    if (!quill) return
    const range = quill.getSelection(true)
  ...
  const modules = useMemo(() => ({
    toolbar: {
      ...
      handlers: { image: imageHandler, link: linkHandler, video: videoHandler, youtube: youtubeHandler },
    },
    history: { delay: 500, maxStack: 200, userOnly: true },
  }), [imageHandler, linkHandler, videoHandler, youtubeHandler])

  // ─── Staged deletion — only executed on Save, supports undo ──────────────
  // Key insight: we never delete immediately. We track what's been removed
  // in a Set. On Ctrl+Z if item comes back, we remove it from the pending set.
  // Only when parent calls flushStagedDeletions() do we actually delete.
  const prevContentRef = useRef(content)
  const stagedImageDeletions = useRef(new Set())
  const stagedVideoDeletions = useRef(new Set())

  const getStorageImages = useCallback((h) => {
    const d = document.createElement('div')
    d.innerHTML = h
    return Array.from(d.querySelectorAll('img'))
      .map(i => i.getAttribute('src'))
  ...
  const getStorageVideos = useCallback((h) => {
    const d = document.createElement('div')
    d.innerHTML = h
    return Array.from(d.querySelectorAll('.ql-video-wrapper[data-type="upload"]'))
      .map(w => w.getAttribute('data-video-src'))
  ...
  // Call this from your Create/Edit page just before saving
  const flushStagedDeletions = useCallback(async () => {
    const imgUrls = [...stagedImageDeletions.current]
    const vidUrls = [...stagedVideoDeletions.current]
    stagedImageDeletions.current.clear()
    stagedVideoDeletions.current.clear()
  ...
  // Expose flush to parent via window so create/edit pages can call it
  useEffect(() => {
    window.__editorFlushDeletions = flushStagedDeletions
    return () => { delete window.__editorFlushDeletions }
  }, [flushStagedDeletions])

  // ─── Restore content — called from create page after draft restore ─────────
  // Must update BOTH Quill's internal content AND prevContentRef
  // so deletion tracking works correctly after restore on page refresh
  useEffect(() => {
    window.__editorRestoreContent = (html) => {
      if (!html) return
      const quill = quillRef.current?.getEditor()
      if (!quill) {
        // Quill not ready yet — retry after a short delay
        setTimeout(() => {
      ...
      prevContentRef.current = html
      // Clear any stale staged deletions from previous session
      stagedImageDeletions.current.clear()
      stagedVideoDeletions.current.clear()
    }
  ...
  // ─── onChange — word/char count + staged deletion tracking ────────────────
  const handleChange = useCallback((html) => {
    ...
    const prevImages = getStorageImages(prevHtml)
    const currImages = getStorageImages(html)
    const prevVideos = getStorageVideos(prevHtml)
    const currVideos = getStorageVideos(html)

    // Images removed → stage for deletion
    prevImages.filter(u => !currImages.includes(u))
      .forEach(u => stagedImageDeletions.current.add(u))
    // Images restored via undo → unstage
    currImages.filter(u => stagedImageDeletions.current.has(u))
      .forEach(u => stagedImageDeletions.current.delete(u))

    // Videos removed → stage for deletion
    prevVideos.filter(u => !currVideos.includes(u))
      .forEach(u => stagedVideoDeletions.current.add(u))
    // Videos restored via undo → unstage
    currVideos.filter(u => stagedVideoDeletions.current.has(u))
      .forEach(u => stagedVideoDeletions.current.delete(u))

    prevContentRef.current = html
    if (onChange) onChange(html)
  }, [onChange, getStorageImages, getStorageVideos])
  ...
  useEffect(() => {
    ...
    const addVideoDeleteButton = (wrapper) => {
      ...
      // In edit mode, user can just backspace to remove — no need for button
